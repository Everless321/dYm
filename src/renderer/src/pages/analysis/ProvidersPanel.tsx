...
import { Plus, Star, Pencil, Trash2, KeyRound, ShieldAlert, Cpu } from 'lucide-react'
import { Button } from '@/components/ui/button'
import type { AiProviderView } from '@shared/ai'
import { ProviderDialog } from './ProviderDialog'
import { protocolLabel } from './shared'

interface ProvidersPanelProps {
  /** 列表变化时通知父级（新建作业对话框要用最新的提供方列表） */
  onChanged?: (providers: AiProviderView[]) => void
}

export function ProvidersPanel({ onChanged }: ProvidersPanelProps): React.JSX.Element {
  ...
  const handleSetDefault = async (id: string): Promise<void> => {
    setBusyId(id)
    try {
      await window.api.ai.setDefaultProvider(id)
      await load()
    } catch (error) {
  ...
  const handleDelete = async (p: AiProviderView): Promise<void> => {
    if (!window.confirm(`删除提供方「${p.name}」？排队中引用它的作业会失败。`)) return
    setBusyId(p.id)
    try {
      await window.api.ai.deleteProvider(p.id)
      toast.success('已删除')
      await load()
    } catch (error) {
  ...
  const openEdit = (p: AiProviderView): void => {
    setEditing(p)
    setDialogOpen(true)
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-[15px] font-semibold text-[#1D1D1F]">AI 提供方</h3>
          <p className="text-xs text-[#A1A1A6] mt-0.5">
            可配置多个服务并切换默认；作业创建时也可单独指定
          </p>
        </div>
      ...
      {providers === null ? (
        ...
        <div className="rounded-2xl border border-dashed border-[#D1D1D6] bg-white py-14 text-center">
          <Cpu className="h-10 w-10 text-[#E5E5E7] mx-auto mb-3" />
          <p className="text-sm text-[#6E6E73]">还没有配置任何 AI 服务</p>
          <p className="text-xs text-[#A1A1A6] mt-1 mb-4">
            支持 OpenAI 兼容接口、Anthropic、Gemini、OpenCode 与 ChatGPT 订阅
          </p>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-[#6E6E73] mt-1">{protocolLabel(p.protocol)}</p>
                  </div>
                ...
                <dl className="mt-4 space-y-1.5 text-xs">
                  <div className="flex gap-2">
                    <dt className="w-12 shrink-0 text-[#A1A1A6]">模型</dt>
                    <dd className="font-mono text-[#1D1D1F] truncate">{p.model || '—'}</dd>
                  </div>
                  {p.protocol !== 'codex' && (
                    <div className="flex gap-2">
                      <dt className="w-12 shrink-0 text-[#A1A1A6]">地址</dt>
                      <dd className="font-mono text-[#6E6E73] truncate">{p.baseUrl}</dd>
                    </div>
                  )}
                  <div className="flex gap-2 items-center">
                    <dt className="w-12 shrink-0 text-[#A1A1A6]">凭据</dt>
                      ...
                      {p.hasCredential ? (
                        <>
                          ...
                          <span className="text-[#1D1D1F]">
                            {p.protocol === 'codex'
                              ? `已登录${p.credentialLabel ? `：${p.credentialLabel}` : ''}`
                              : '已保存'}
                          </span>
                        </>
                      ) : (
                        <>
                          ...
                          <span className="text-[#6E6E73]">
                            {p.protocol === 'codex' ? '未登录' : '未设置（本地服务可不填）'}
                          </span>
                        </>
                      )}
                    </dd>
