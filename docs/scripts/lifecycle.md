...
钩子触发走同一套运行集合：某个脚本的手动运行、cron、钩子不会并发。
钩子事件会排队，点「停止」会连队列一起清掉。详见[事件钩子](/scripts/hooks)。
...
每个脚本可以自己设日志留存条数（默认 1000，最少 50，最多 20000），
写在用户数据目录的 `script-logs/` 里，**关掉应用再打开还在**。
「输出」按**每次执行**列出，点进去看这一次的日志。点「清空」会把内存和磁盘一起删掉。删除脚本时日志文件也会去掉；重命名会跟着搬走。

钩子脚本跑失败后，页面上的「再次运行」会带上**上次那次钩子的入参**再执行，不用等下一条作品。
语法都加载不出来的脚本不能跑，改到能保存通过之后就可以。
